...
export function CamPage({ items, onSave, programs }: { items: MarketplaceItem[]; onSave: (value: CamSave) => void | Promise<void>; programs: ProgramSettings }) {
  ...
  return <main className="cam-page">
    ...
    <div className="cam-settings">
      <label>Material thickness<select value={settings.profilePasses === 2 ? '12-2pass' : settings.thickness} onChange={e => update(e.target.value === '12-2pass' ? { thickness: 12, profilePasses: 2 } : { thickness: Number(e.target.value) as CamSettings['thickness'], profilePasses: undefined })}><option value="18">18 mm</option><option value="15">15 mm</option><option value="12">12 mm (1 pass)</option><option value="12-2pass">12 mm (2 passes)</option><option value="6">6 mm (1 pass)</option></select></label>
