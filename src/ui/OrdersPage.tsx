...
import { cuttingPayment, OrderAssignmentForm } from './OrderAssignmentForm'
    ...
    void (async () => {
      try {
        ...
        if (connection.connected) {
          const params = new URLSearchParams({ status: connection.isOrderAdmin ? status : 'all', ...(search ? { search } : {}), ...(after ? { after } : {}) })
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        setDetailError('')
      } catch (e) { if (!abort.signal.aborted) { setDetail(undefined); setDetailError((e as Error).message) } }
      finally { if (!abort.signal.aborted) setDetailLoaded(detailKey) }
    })()
    return () => abort.abort()
  }, [userId, selected, detailCursor, detailRetry, detailKey])

  function closeDetail() { setSelected(''); setDetail(undefined); setDetailCursor(''); setDetailError(''); setAssignmentSaved(false) }
  function refresh() { closeDetail(); setCursors([]); setRevision(v => v + 1) }
  return <main className="queue-page orders-page">
    <div className="queue-heading"><div><h2>{connection?.isOrderAdmin ? 'Orders' : 'My orders'}</h2>{connection?.shop && <p className="orders-store">{connection.shop}</p>}</div><button type="button" className="icon-button" title="Refresh orders" aria-label="Refresh orders" disabled={loading} onClick={refresh}><RefreshCw size={18} /></button></div>
    ...
    {connection && !connection.connected && !loading && !error ? <p>No Shopify store connected to this account.</p> : <>
      <form className="orders-filters" onSubmit={e => { e.preventDefault(); closeDetail(); setCursors([]); setSearch(draft.trim()); setRevision(v => v + 1) }}>
        {connection?.isOrderAdmin && <label>Status<select value={status} onChange={e => { closeDetail(); setStatus(e.target.value); setCursors([]) }}><option value="open">Open orders</option><option value="all">All orders</option><option value="unfulfilled">Unfulfilled / partial</option><option value="fulfilled">Fulfilled</option><option value="cancelled">Cancelled</option></select></label>}
      ...
      {!loading && data && <>
        <div className="queue-table-scroll"><table className="queue-table orders-table"><thead><tr><th>Order</th><th>Items ordered</th>{connection?.isOrderAdmin && <th>Assigned to</th>}<th>Cutting payment</th>{sheetTarget && <th>Sheet</th>}</tr></thead><tbody>{data.orders.map(order => <tr key={order.id} className={orderId(order) === selected ? 'selected-job' : ''}>
          ...
          {connection?.isOrderAdmin && <td className="order-assignee">{order.assignment?.assignee_email ?? (order.assignment?.assignee_id ? 'Assigned user' : 'Unassigned')}<button type="button" className="order-assign-button" aria-label={`Assign ${order.name}`} onClick={() => { closeDetail(); setSelected(orderId(order)); setDetailRetry(v => v + 1) }}>Assign</button></td>}
          <td><span className="order-payment-label">Cutting payment: </span>{cuttingPayment(order.assignment)}</td>
        ...
        {!data.orders.length && <p>{connection?.isOrderAdmin ? "No matching orders within the Shopify app's accessible history." : 'No assigned orders found.'}</p>}
      ...
      {selected && <section className="orders-detail" aria-label="Order details">
        ...
        {detail && <>
          {connection?.isOrderAdmin ? <OrderAssignmentForm key={`${userId}:${selected}:${detail.order.assignment?.version ?? 0}`} userId={userId} orderId={selected} assignment={detail.order.assignment}
            onReload={() => { setDetail(undefined); setDetailCursor(''); setDetailRetry(v => v + 1); setAssignmentSaved(false) }}
            onSaved={assignment => {
              setDetail(previous => previous ? { ...previous, order: { ...previous.order, assignment } } : previous)
              setData(previous => previous ? { ...previous, orders: previous.orders.map(o => orderId(o) === selected ? { ...o, assignment } : o) } : previous)
              setAssignmentSaved(true)
            }} /> : <p>Cutting payment: <strong>{cuttingPayment(detail.order.assignment)}</strong></p>}
          {assignmentSaved && <p role="status">Assignment saved.</p>}
