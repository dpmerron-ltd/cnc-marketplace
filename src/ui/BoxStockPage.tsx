...
export function BoxStockPage({ userId, items, parts }: { userId: string; items: MarketplaceItem[]; parts: Part[] }) {
  ...
  const ownItems = items.filter(i => i.ownerId === userId && parts.some(p => p.ownerId === userId && p.itemId === i.id))
  const jobs = stock.boxes ? JSON.stringify(ownItems.map(item => ({ id: item.id, pieces: packingPieces(parts.filter(p => p.ownerId === userId && p.itemId === item.id), item.packing), settings: item.packing, boxes: stock.boxes }))) : undefined
  ...
  return <main className="queue-page box-stock-page">
    ...
    {stock.boxes && <><div className="queue-table-scroll"><table className="queue-table box-inventory"><thead><tr><th>Box</th><th>Internal dimensions</th><th>In stock</th></tr></thead><tbody>{stock.boxes.map(box => <tr key={box.id}><td>{box.name}<span className="stock-mobile-dimensions">{boxSize({ length: box.length_mm, width: box.width_mm, height: box.height_mm })} internal</span><span className="queue-order">{box.details}</span></td><td>{boxSize({ length: box.length_mm, width: box.width_mm, height: box.height_mm })}</td><td><StockCount key={`${box.id}:${box.version}`} box={box} userId={userId} onSaved={stock.reload} /></td></tr>)}</tbody></table></div>{!stock.boxes.length && <p>No shared box sizes.</p>}
      <section className="box-fit-section"><h3>Item Packing</h3><div className="queue-table-scroll"><table className="queue-table"><thead><tr><th>Item</th><th>Recommended box</th><th>Per-item availability</th></tr></thead><tbody>{ownItems.map(item => { const result = estimates[item.id]; return <tr key={item.id}><td>{item.name}<span className="queue-order">{item.sku}</span></td><td>{result?.result?.boxes.length ? result.result.boxes.map((b, i) => <div key={i}>{b.stockName || boxSize(b.internal)}{!b.stockId ? ' (new size)' : ''}</div>) : result?.error || result?.result?.errors.join(' ') || 'Calculating...'}</td><td>{result?.result?.boxes.length ? result.result.boxes.every(b => b.stockId) ? result.result.warnings.some(w => w.startsWith('Replenish')) ? 'Restock required' : `${result.result.boxes.length} box${result.result.boxes.length === 1 ? '' : 'es'} available` : 'Order new size' : ''}</td></tr> })}</tbody></table></div></section>
      <section className="box-fit-section"><h3>Suggested Sizes To Order</h3>{[...suggestions].length ? <ul className="box-suggestions">{[...suggestions].sort((a, b) => b[1].length - a[1].length).map(([size, names]) => <li key={size}><strong>{size} internal</strong><span>{names.join(', ')}</span></li>)}</ul> : <p>{ownItems.some(i => !estimates[i.id]) ? 'Calculating suggestions...' : 'No additional size suggested from the current components.'}</p>}</section>
    </>}
  </main>
}
