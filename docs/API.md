...
Send `Authorization: Bearer <account API key>` on every request. A verified Supabase user access token with MFA (`aal2`) is also accepted for the operator UI. Do not use the project's public key as authentication, and never give integrations the service-role key or Supabase personal access token. Account API keys can read that account's catalog/jobs/files, create catalogue items, upload new components, replace/remove item images and create or transition jobs. They cannot overwrite/delete components or manage keys.
...
Generate and retain a UUID `id` per physical component design. Name is limited to 200 characters, SKU to 100, and filename to 128 (no paths; `.nc`, `.tap`, `.gcode`, `.cnc`). NC and optional source DXF are each limited to 2 MB UTF-8; NC may have at most 10,000 lines, and JSON body at most 4 MiB. NC must explicitly use G21/G90, contain finite machining geometry, and pass the existing transform and simulation error checks. Geometry, bounds and owner are derived by the server; caller-supplied dimensions/owner IDs are rejected. The optional DXF is retained as source, not regenerated by this endpoint.
