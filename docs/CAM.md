...
Profiles offset outside the drawing by 3.175 mm. Door/internal contours offset inside by 3.175 mm. Every through-cut part or hole larger than 12 mm in either X or Y requires holding tabs, including circular openings. Size is measured on the original outline in millimetres before cutter compensation. Recognized doors always require tabs. These contours default to four tabs and accept counts from one to four; smaller contours may fit fewer. Zero-tab overrides are rejected, and export is blocked if no tab fits. Other contours at most 12 mm in both directions default to zero tabs. Blind pockets and drills have no tabs. Tabs are kept clear of every entry and re-entry ramp. When a clear span is short, the same 3-degree descent uses additional out-and-back cycles. Below-surface tab lifts use feed moves, not rapids.
