...
Profiles offset outside the drawing by 3.175 mm. Door/internal contours offset inside by 3.175 mm. Through-cut parts and holes larger than 12 mm in either X or Y default to four tabs, including circular openings. Size is measured on the original outline in millimetres before cutter compensation. Non-door inside holes/cut-outs accept an explicit zero-tab override, with an independent-workholding review notice. Recognized doors always require tabs, as do outside profiles larger than 12 mm: these accept one to four tabs and block export if no tab fits. Smaller contours may fit fewer tabs. Other contours at most 12 mm in both directions default to zero tabs. Blind pockets and drills have no tabs. Tabs are kept clear of every entry and re-entry ramp. When a clear span is short, the same 3-degree descent uses additional out-and-back cycles. Below-surface tab lifts use feed moves, not rapids.
