...
Confirmed components save in the background, one at a time, while the next DXF opens immediately. **Component saves** remains visible when navigating to other pages, with queued/saving/saved/failed states and per-file retry. Retries retain the exact confirmed NC, variants, item, SKU and component ID. Failed saves do not stop later confirmed files. Confirmed is not the same as saved: keep the tab open until all saves finish. Reload/close warns while queued, in-flight or failed saves remain; signing out asks before discarding them. Pending saves are held in memory, not restored after reload, and never transfer between accounts.
...
Profiles offset outside the drawing by 3.175 mm. Door/internal contours offset inside by 3.175 mm. Internal openings whose narrowest full-outline width is at most 12 mm never have tabs, even with a positive override or a door label. Rectangular slots, including those with local corner reliefs, use straight-wall spacing up to 12.2 mm, allowing 0.2 mm fit clearance on a nominal 12 mm mortise. The relief outline and toolpath geometry are not changed by this measurement. Width is measured in millimetres before compensation, independent of rotation; circles retain a strict 12 mm diameter threshold. Other inside openings default to four tabs. If none fit an ordinary internal cutout, tabs are automatically removed with a waste-movement review warning. Non-door inside cuts also accept an explicit zero-tab override. Larger doors and outside profiles retain mandatory holding tabs and block export when none fit. Outside profiles use their existing 12 mm X/Y threshold. Blind pockets and drills have no tabs. Tabs are kept clear of every entry and re-entry ramp. When a clear span is short, the same 3-degree descent uses additional out-and-back cycles. Below-surface tab lifts use feed moves, not rapids.
