...
Profiles offset outside the drawing by 3.175 mm. Door/internal contours offset inside by 3.175 mm. Outer profiles and layers containing `DOOR` default to four holding tabs; other internal openings, including circular `CUT_INNER` holes, default to no tabs and are cut through, not pocket-cleared. Review workholding and loose offcuts for tab-free openings. Tab counts from zero to four can be selected per contour; smaller contours may fit fewer. Tabs are kept clear of every entry and re-entry ramp. When a clear span is short, the same 3-degree descent uses additional out-and-back cycles. Below-surface tab lifts use feed moves, not rapids.
...
Hinge detection uses geometry: a 35 mm circle (0.02 mm diameter tolerance) fully contained by a non-circular inside/door contour is a hinge pocket. The enclosing contour is treated as a door with holding tabs, including drawings on layer `0`. Hinge pockets default to 12 mm blind depth in 18 mm stock and are blocked in 12 mm stock, even if their depth is overridden. Other internal openings default to through-cuts without tabs, including non-hinge geometry on `POCKET`/`HINGE` layers. Layer depth hints alone do not decide which geometry is a hinge.
